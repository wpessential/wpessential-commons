...
class Sanitize
{
	...
	public static function editor ( $input, $force_filter = false, $auto_p = false, $allowed_tags = null )
	{
		...
		{
			global $allowedtags;
			$output = wp_kses( trim( $input ), apply_filters( 'wpe/sanitize/editor_tags', $allowed_tags ?? $allowedtags ) );
		}
	...
	{
		$tags   = apply_filters( 'wp/sanitize/html_guest_tags_' . ( $namespace ?? 'default' ), $allowed_tags ?? [
			'em'         => [],
			'strong'     => [],
			'small'      => [],
			'sub'        => [],
	...
	{
		$tags   = apply_filters( 'wpe/sanitize/html_tags_' . ( $namespace ?? 'default' ), $allowed_tags ?? [
			'em'         => [ 'class' => true ],
			'strong'     => [ 'class' => true ],
			'small'      => [],
			'sub'        => [ 'class' => true ],
