...
if ( ! function_exists( 'wpe_get_post_types' ) )
{
	...
	function wpe_get_post_types ( $args = [], $output = 'objects', $empty = false, $public = true )
	{
		...
		{
			$items[ 'no' ] = esc_html__( 'No Post Type Found', 'TEXT_DOMAIN' );
		}
...
if ( ! function_exists( 'wpe_get_taxonomies' ) )
{
	...
	function wpe_get_taxonomies ( $args = [], $output = 'names', $empty = false, $public = true )
	{
		...
		{
			$items[ 'no' ] = esc_html__( 'No Taxonomy Found', 'TEXT_DOMAIN' );
		}
...
if ( ! function_exists( 'wpe_get_terms' ) )
{
	...
	function wpe_get_terms ( $args = [], $empty = false, $slug = false )
	{
		...
		{
			$items[ 'no' ] = esc_html__( 'No Term Found', 'TEXT_DOMAIN' );
		}
...
if ( ! function_exists( 'wpe_get_nav_menus' ) )
{
	...
	function wpe_get_nav_menus ( $args = [], $value_field = 'id', $empty = false )
	{
		...
		{
			$items[ 'no' ] = esc_html__( 'No Menu Found', 'TEXT_DOMAIN' );
		}
...
if ( ! function_exists( 'wpe_get_page_title' ) )
{
	...
	function wpe_get_page_title ( bool $include_context = true )
	{
		...
		{
			/* translators: %s: Search term. */
			$title = sprintf( esc_html__( 'Search Results for: %s', 'TEXT_DOMAIN' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
			{
				/* translators: %s is the page number. */
				$title .= sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'TEXT_DOMAIN' ), get_query_var( 'paged' ) );
			}
		}
		elseif ( is_category() )
		{
			...
			if ( $include_context )
			{
				...
				$title = sprintf( esc_html__( 'Category: %s', 'TEXT_DOMAIN' ), $title );
			}
		}
		elseif ( is_tag() )
		{
			...
			if ( $include_context )
			{
				...
				$title = sprintf( esc_html__( 'Tag: %s', 'TEXT_DOMAIN' ), $title );
			}
		}
		elseif ( is_author() )
		{
			...
			if ( $include_context )
			{
				...
				$title = sprintf( esc_html__( 'Author: %s', 'TEXT_DOMAIN' ), $title );
			}
		}
		elseif ( is_year() )
		{
			...
			if ( $include_context )
			{
				...
				$title = sprintf( esc_html__( 'Year: %s', 'TEXT_DOMAIN' ), $title );
			}
		}
		elseif ( is_month() )
		{
			...
			if ( $include_context )
			{
				...
				$title = sprintf( esc_html__( 'Month: %s', 'TEXT_DOMAIN' ), $title );
			}
		}
		elseif ( is_day() )
		{
			...
			if ( $include_context )
			{
				...
				$title = sprintf( esc_html__( 'Day: %s', 'TEXT_DOMAIN' ), $title );
			}
		}
		elseif ( is_tax( 'post_format' ) )
		{
			...
			{
				$title = _x( 'Asides', 'post format archive title', 'TEXT_DOMAIN' );
			}
			elseif ( is_tax( 'post_format', 'post-format-gallery' ) )
			{
				$title = _x( 'Galleries', 'post format archive title', 'TEXT_DOMAIN' );
			}
			elseif ( is_tax( 'post_format', 'post-format-image' ) )
			{
				$title = _x( 'Images', 'post format archive title', 'TEXT_DOMAIN' );
			}
			elseif ( is_tax( 'post_format', 'post-format-video' ) )
			{
				$title = _x( 'Videos', 'post format archive title', 'TEXT_DOMAIN' );
			}
			elseif ( is_tax( 'post_format', 'post-format-quote' ) )
			{
				$title = _x( 'Quotes', 'post format archive title', 'TEXT_DOMAIN' );
			}
			elseif ( is_tax( 'post_format', 'post-format-link' ) )
			{
				$title = _x( 'Links', 'post format archive title', 'TEXT_DOMAIN' );
			}
			elseif ( is_tax( 'post_format', 'post-format-status' ) )
			{
				$title = _x( 'Statuses', 'post format archive title', 'TEXT_DOMAIN' );
			}
			elseif ( is_tax( 'post_format', 'post-format-audio' ) )
			{
				$title = _x( 'Audio', 'post format archive title', 'TEXT_DOMAIN' );
			}
			elseif ( is_tax( 'post_format', 'post-format-chat' ) )
			{
				$title = _x( 'Chats', 'post format archive title', 'TEXT_DOMAIN' );
			}
		}
		elseif ( is_post_type_archive() )
		{
			...
			if ( $include_context )
			{
				...
				$title = sprintf( esc_html__( 'Archives: %s', 'TEXT_DOMAIN' ), $title );
			}
		}
		elseif ( is_tax() )
		{
			...
			if ( $include_context )
			{
				...
				$title = sprintf( esc_html__( '%1$s: %2$s', 'TEXT_DOMAIN' ), $tax->labels->singular_name, $title );
			}
		}
		elseif ( is_404() )
		{
...
if ( ! function_exists( 'wpe_get_post_custom_keys_array' ) )
{
	...
	function wpe_get_post_custom_keys_array ( bool $is_multiselect = false )
	{
		...
		{
			$items[ 'no' ] = esc_html__( 'No Key Found', 'TEXT_DOMAIN' );
		}
